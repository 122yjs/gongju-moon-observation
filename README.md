...
Google Sheets의 원본 `관찰 기록`에는 학생이 처음 설정한 관찰 시각과 서버가 실제 제출 요청을 처리한 시각을 별도 열로 보존합니다. 기존 제출 행도 이미 저장된 두 값을 그대로 사용하며, 교사 화면을 열거나 `제출 목록`을 열면 최신 헤더와 한국시간 요약 열이 자동으로 적용됩니다. 교사가 관찰 시각을 정정해도 학생의 최초 입력값은 덮어쓰지 않고 별도 정정 시각·정정 시점·사유 이력을 같은 Sheets에 남깁니다. 교사 화면에서는 학생 설정 시각과 실제 제출 시각의 차이가 2시간 이상이거나 학생 설정 시각이 제출 시각보다 5분 이상 미래이면 `확인 필요`로 표시합니다.
