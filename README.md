...
JPEG 사진에 EXIF 촬영 시각이 있거나 페이지 안 카메라로 바로 촬영한 경우, 학생 화면에서 촬영 시각과 입력한 관찰 시각을 기기 안에서 비교합니다. 차이가 30분 이내면 자연스러운 관찰·제출 간격으로 보고 아무 경고도 띄우지 않습니다. 30분을 넘으면 촬영 시각 적용, 직접 수정, 입력값 유지 중 하나를 학생이 명시적으로 선택한 뒤 제출할 수 있습니다. EXIF에서는 촬영 시각만 제한적으로 읽고 GPS 등 다른 메타데이터는 수집하지 않으며, 제출용 JPEG 재인코딩 과정에서 메타데이터는 제거됩니다. 촬영 시각 원본 자체를 중앙 D1이나 Google Sheets의 별도 필드로 전송하지 않습니다.
